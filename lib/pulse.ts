...
async function buildSnapshot(range: RangeKey): Promise<Snapshot> {
  ...
  const emptyGh = {
    repos: [] as Repo[],
    sources: [
      { id: "github-trending", ok: false, status: 0, error: "unavailable", count: 0 },
      { id: "github-search", ok: false, status: 0, error: "unavailable", count: 0 },
    ],
  ...
  const emptyHf = {
    ...
    spaces: [] as HfItem[],
    sources: [
      { id: "hf-models", ok: false, status: 0, error: "unavailable", count: 0 },
      { id: "hf-datasets", ok: false, status: 0, error: "unavailable", count: 0 },
      { id: "hf-spaces", ok: false, status: 0, error: "unavailable", count: 0 },
    ],
    fetchedAt: new Date().toISOString(),
  };
  const emptyArxiv = {
    papers: [] as Paper[],
    status: { id: "arxiv", ok: false, status: 0, error: "unavailable", count: 0 },
    fetchedAt: new Date().toISOString(),
  };
